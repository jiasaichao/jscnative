  ...
  render() {
    return (
      <View style={styles.container}>
        <FlatList data={this.state.items} renderItem={this.renderItem} keyExtractor={(_, index) => `${index}`} />
      </View>
    );
  }
    ...
    return (
      <View style={styles.container}>
        <Transition shared={`image${this.props.navigation.state.params.index}`}>
          <Image style={styles.largeImage} source={{ uri: this.props.navigation.state.params.item.url }} />
        </Transition>
